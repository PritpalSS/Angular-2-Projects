...
@Injectable()  //adding Injectable decorator is important for any service that you are going to inject into your components or another service
export class EventService {

	constructor(private http:Http) {}

	getEvents():Observable<IEvent[]> {  //With Http, the data that is returned is Observable data
		let subject = new Subject<IEvent[]>();  //Subject is a type of observable 
		setTimeout(() => {subject.next(EVENTS); subject.complete(); }, 100);  //subject.next(EVENTS) <-- Adding data to observable stream
		return subject;
	}
